    ...
    <section className="bg-white py-24 lg:py-32">
      <div className="max-w-7xl mx-auto px-6 lg:px-8" ref={ref}>
        ...
        <motion.h2 className="font-display font-normal text-[clamp(28px,4vw,52px)] text-dark leading-tight uppercase mb-14" initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : undefined} transition={{ duration: 0.7, ease: 'easeOut' }}>
          {heading}{' '}<span className="text-teal-accent">{headingAccent}</span>
        </motion.h2>

        <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12 mb-12" variants={staggerContainer} initial="hidden" animate={isInView ? 'visible' : 'hidden'}>
          {painPoints.map((point, i) => {
            ...
            return (
              <motion.div key={i} variants={fadeUp} className="text-center px-4">
                ...
                <h3 className="font-display font-normal text-[18px] text-dark uppercase tracking-wide mb-3">{point.title}</h3>
                <p className="font-body text-muted text-[15px] leading-relaxed">{point.description}</p>
              </motion.div>
            )
          })}
        </motion.div>

        <motion.p className="text-center font-display font-normal text-lg text-teal-accent uppercase tracking-wide" initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : undefined} transition={{ delay: 0.6, duration: 0.7 }}>
          {closingLine}
        </motion.p>
      </div>
    </section>
