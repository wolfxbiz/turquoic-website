// ── templates/landing-page/components/TemplateFooter.tsx ──
    ...
    <footer className="bg-dark text-white border-t-2 border-teal-strong py-16">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 pb-8 border-b border-white/10">
          <a href="#" className="font-display font-normal text-4xl lg:text-5xl text-white tracking-tight uppercase">
            {config.brand.name}
          </a>
          ...
          <nav className="flex items-center gap-6" aria-label="Footer navigation">
            {config.navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="font-body text-[15px] font-normal text-white/60 hover:text-teal-strong transition-colors uppercase"
              >
                {link.label}
              </a>
            ))}
